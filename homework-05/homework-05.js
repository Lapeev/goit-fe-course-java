...
const users = [
  ...
  const getTotalBalance = arr => arr.reduce((acc, member) => acc + member.balance, 0 );
  console.log(getTotalBalance(users));
  
  const getUsersByFriend = (arr, name) => arr.reduce((acc, member) => 
                                                     member.friends.includes(name) ?
                                                    acc.concat(member.name) :
                                                    acc, []);
  console.log(getUsersByFriend(users, 'Briana Decker'));
  console.log(getUsersByFriend(users, 'Goldie Gentry'));
  
  const getAllSkills = arr => arr.reduce((acc, member) => acc.concat(member.skills) , [])
                                 .reduce((acc, prof) =>!acc.includes(prof) ?
                                         acc.concat(prof) :
                                         acc, [])
                                 .sort();
  ...
  const getUserNamesSortedByFriendsCount = arr => arr.sort((a, b) => a.friends.length - b.friends.length)
                                                     .reduce((acc, member) => acc.concat(member.name) , []);
